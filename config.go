...
type (
	...
	Config struct {
		...
		ServiceManager   *ServiceManagerConfig `yaml:"ServiceManager,omitempty"`   // 服务管理配置
		Logger           *LoggerConfig         `yaml:"Logger,omitempty"`           // 日志配置
	}
	...
	// ServiceManagerConfig 服务管理配置
	ServiceManagerConfig struct {
		Prefix      string   `yaml:"Prefix"` // 前缀
		EtcdGateway []string `yaml:"EtcdGateway"`
		LeaseTTL    int      `yaml:"LeaseTTL"`
		HeartT      int      `yaml:"HeartT"`
	}

	// LoggerConfig 日志配置
	LoggerConfig struct {
		Level      string `yaml:"Level"`      // 输出的日志级别, 默认debug
		OutPath    string `yaml:"OutPath"`    // 日志文件输出路径，默认logs
		MaxSize    int    `yaml:"MaxSize"`    // 文件最大大小 MB，默认50M
		CallerSkip int    `yaml:"callerSkip"` // 跳过多少层次，找caller
	}
)
